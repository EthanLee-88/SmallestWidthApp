...
import com.blues.smallestwidthapp.ui.video.glutilsOld.GLDrawer2D;
...
public final class CameraGLView extends GLSurfaceView {
    ...
    private static BestPreViewSizeListener mBestPreViewSizeListener;

    public interface BestPreViewSizeListener {
        void bestSize(Camera.Size size);
    }

    public void setBestPreViewSizeListener(BestPreViewSizeListener listener) {
        mBestPreViewSizeListener = listener;
    }

    private static void setBestSize(Camera.Size size) {
        if (mBestPreViewSizeListener == null) {
            return;
        }
        mBestPreViewSizeListener.bestSize(size);
    }
...
/*		// the frequency of refreshing of camera preview is at most 15 fps
    ...
    public void resizeCameraView(Camera.Size size) {
        post(() -> {
            if ((size.width <= 0) || (size.height <= 0)) {
                return;
            }
            int newHeight = size.width * getWidth() / size.height;
            int deltaHeight = getHeight() - newHeight;
            Log.d(TAG, "deltaHeight = " + deltaHeight + " - size = " + size.width + " - " + size.height);
            if (deltaHeight <= 50) {
                return;
            }
            ((ViewGroup.MarginLayoutParams) getLayoutParams()).bottomMargin = deltaHeight - deltaHeight / 10;
            requestLayout();
        });
    }

    public void reSetVideoSize() {
        post(() -> setVideoSize(getWidth(), getHeight()));
    }
    ...
    public void onResume() {
        ...
        if (mHasSurface) {
            if (mCameraHandler == null) {
                ...
                post(() -> {
                    Log.d(TAG, "pre size = " + getWidth() + " - h = " + getHeight());
...
//					}
                    ...
                    final Camera.Size closestSize = getBestPreviewSize(
                    ...
                    final Camera.Size pictureSize = getBestPreviewSize(
                            params.getSupportedPictureSizes(), width, height);
                    params.setPictureSize(pictureSize.width, pictureSize.height);
                    setBestSize(closestSize);
//                    getBestPreviewSize(params.getSupportedPreviewSizes(), width, height);
                    ...
                    Log.i(TAG, String.format("previewSize(%d, %d)", previewSize.width, previewSize.height)
                            + "closestSize = " + closestSize.width + " - " + closestSize.height + "\n"
                            + "pictureSize = " + pictureSize.width + " - " + pictureSize.height + "\n"
                            + "set size = " + width + " h = " + height);
        ...
        private static Camera.Size getBestPreviewSize(List<Camera.Size> supportedSizes, final int requestedWidth, final int requestedHeight) {
            double setRate = (double) requestedHeight / (double) requestedWidth;
            Log.d(TAG, "getBestPreviewSize setRate = " + setRate);
            Camera.Size bestSize = supportedSizes.get(0);
            for (Camera.Size supportedSize : supportedSizes) {
                double currentRate = (double) supportedSize.width / (double) supportedSize.height;
                double preRate = (double) bestSize.width / (double) bestSize.height;
                Log.d(TAG, "getBestPreviewSize w = " + supportedSize.width + " - h = "
                        + supportedSize.height + "pre Rate = " + preRate + " - currentRate = " + currentRate);

                if ((supportedSize.width < 1000) || (currentRate > setRate)) {
                    continue;
                }
                double currentDeltaRate = setRate - currentRate;
                double preDeltaRate = setRate - preRate;
                if (preDeltaRate <= 0) { //前一个太大，放弃
                    bestSize = supportedSize;
                    if (preDeltaRate == 0) {
                        break;
                    }
                } else if ((currentDeltaRate <= preDeltaRate) || ((currentDeltaRate <= preDeltaRate + 0.01) && (bestSize.width > supportedSize.width))) { //当前差距更小，取值
                    bestSize = supportedSize;
                }
            }
            Log.d(TAG, "getBestPreviewSize best = " + bestSize.width + " - " + bestSize.height);
            return bestSize;
        }
